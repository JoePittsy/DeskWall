...
| `src/DeskWall.Designer/` | `DeskWall.Designer.exe`, WPF, JIT (not AOT). `Model/` (document, undo, live source panels, settings), `Views/` (canvas, panels, XAML). Main window shell still in progress (Phase 5 Task 8). |
...
| `layouts/` | Starter and example layout JSON files, plus `layouts/README.md`. |
